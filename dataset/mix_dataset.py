...
from utils.common import bgr2ycbcr
...
class MixDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_hr = cv2.imread(url_hr, cv2.IMREAD_COLOR).astype(np.float32) / 255.0
        img_lr = cv2.imread(url_lr, cv2.IMREAD_COLOR).astype(np.float32) / 255.0
        ...
        img_lr = bgr2ycbcr(img_lr, only_y=True)
        img_hr = bgr2ycbcr(img_hr, only_y=True)
        ...
        img_hr = torch.from_numpy(img_hr).unsqueeze(0)
