    ...
    if args.sr_type == 'SISR':
        with torch.no_grad():
            for i, f in enumerate(ipath_l):
                ...
                img_lr = cv2.imread(f, cv2.IMREAD_COLOR).astype(np.float32) / 255.0
                img_lr = bgr2ycbcr(img_lr, only_y=True)
                img_lr = torch.from_numpy(img_lr).unsqueeze(0).unsqueeze(0).to(device)
                B, C, H, W = img_lr.size()
                crop_H = False
                crop_W = False
                # print('img lr size', img_lr.size())
                if W % 2 == 1:
                    crop_W = True
                    img_lr = img_lr[:,:,:,:-1]
                if H % 2 == 1:
                    ...
                    img_lr = img_lr[:,:,:-1,:]
                # print('img lr size', img_lr.size())
                output = model(img_lr).squeeze().float().cpu().clamp_(0,1)
